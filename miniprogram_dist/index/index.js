const main = {
  ...
  drawImage(data) {
    ...
    this.ctx.save()
    if (borderRadius > 0) {
      this._drawRadiusRect(x, y, w, h, borderRadius, isCustom, isBttomCustom)
      this.ctx.clip()
      this.ctx.drawImage(
        imgPath,
        this.toPx(sx),
...
const handle = {
   ...
   * 画圆角矩形
   */
  _drawRadiusRect(x, y, w, h, r, isCustom, isBttomCustom) {
    const br = r / 2
    this.ctx.beginPath()
    if (isCustom) {
      ...
      this.ctx.lineTo(this.toPx(x + w), this.toPx(y + h))
      this.ctx.lineTo(this.toPx(x), this.toPx(y + h))
  ...
  _drawSingleText({
    ...
    if (textWidth > width) {
      ...
      if (textArr.length * unitLineNum < text.length) {
        ...
        textArr[textArr.length - 1] = textArr[textArr.length - 1].replace(
          reg,
          '...'
        )
      }
      ...
      this._downImage(imageUrl, index)
        ...
        .then(({ imgPath, imgInfo }) => {
          ...
          this.drawArr.push({
            ...
            isBttomCustom: image.isBttomCustom || false,
          })
          resolve()
        })
